...
export function isTopicCommand(text) {
  return /^\/(topic|issue|hot|keyword|주제|토픽|이슈)(@\w+)?\s+/i.test(text.trim()) || /^(주제|토픽|이슈|핫이슈|키워드)\s*[:：]/i.test(text.trim());
}

export function extractTopicKeyword(text) {
  return text
    .replace(/^\/(topic|issue|hot|keyword|주제|토픽|이슈)(@\w+)?\s+/i, "")
    .replace(/^(주제|토픽|이슈|핫이슈|키워드)\s*[:：]\s*/i, "")
    .replace(/\s+/g, " ")
    .trim();
}
