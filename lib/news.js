...
export async function collectMorningNews() {
  const groups = await Promise.all(
    MORNING_QUERIES.map(([section, query]) => fetchRssItems(query, section))
  );
...
export async function collectTrendingNews() {
  const groups = await Promise.all(
    TREND_QUERIES.map(([section, query]) => fetchRssItems(query, section))
  );
...
export async function collectKeywordNews(keyword) {
  ...
  const groups = await Promise.all(
    queries.map((query) => fetchRssItems(query, "키워드 뉴스"))
  );
