import { collectKeywordNews, collectMorningNews, collectTrendingNews, extractSourceButtons, removeRawUrls } from "./news.js";
import { linkButtonLimit } from "./config.js";
import { buildPersonKeywordWarning } from "./person-guard.js";
...
import { sendChatAction, sendInlineKeyboardMessage, sendMessage, sendPhoto } from "./telegram.js";
...
export async function sendMorningBriefing(chatId, replyToMessageId = undefined) {
  ...
  const sourceButtons = extractSourceButtons(sourceText, linkButtonLimit());
  const response = await createTextResponse(buildMorningBriefingPrompt(sourceText));
  const briefing = removeRawUrls(getOutputText(response));

  await sendMessage(chatId, briefing, { replyToMessageId });

  if (sourceButtons.length) {
    await sendInlineKeyboardMessage(chatId, "🔗 참고 기사 바로가기", sourceButtons, {
      replyToMessageId
    });
  }
}
...
export async function sendKeywordBriefing(chatId, keyword, replyToMessageId = undefined) {
  ...
  const sourceButtons = extractSourceButtons(sourceText, linkButtonLimit());
  const personWarning = buildPersonKeywordWarning(keyword, sourceText);
  const response = await createTextResponse(buildKeywordBriefingPrompt(keyword, sourceText, personWarning));
  const answer = removeRawUrls([personWarning, getOutputText(response)].filter(Boolean).join("\n"));
...
export async function sendTrendRanking(chatId, replyToMessageId = undefined) {
  ...
  const ranking = removeRawUrls(getOutputText(response));
...
async function createAndSaveCardScript(chatId, keyword) {
  ...
  const scriptResponse = await createTextResponse(buildCardScriptPrompt(keyword, sourceText, buildPersonKeywordWarning(keyword, sourceText)));
